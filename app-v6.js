...
async function init(){
  const data=await fetch('data.json?v=6').then(r=>r.json());
...
function refreshMapMarkers(){
  ...
  state.mapMarkers.forEach(({marker,item,kind})=>{
    ...
    const ratingOk=kind==='activity'||!state.mapRating||String(item.rating)===String(state.mapRating);
...
function saveTravelRecord(id,form){
  ...
  renderTimeline();renderLocationActions(id,all[id]);
}
...
function renderTimeline(){
  ...
  for(const [id,label] of [['flight-sebastien','Sébastien'],['flight-audrey','Audrey']]){
    const f=d[id]||{};
    add(f.departure,'✈️',`${label} · départ`,[f.flightNumber,f.from&&`depuis ${f.from}`,f.to&&`vers ${f.to}`].filter(Boolean).join(' · '));
    add(f.arrival,'🛬',`${label} · arrivée`,[f.flightNumber,f.to].filter(Boolean).join(' · '));
  }
...
function renderLocationActions(id,data){
  ...
  let address='';if(id.startsWith('airbnb'))address=data.address||'';else address=data.pickupAddress||data.returnAddress||'';
...
function initTravel(){loadTravelForms();initDocuments();renderTimeline();document.querySelector('#exportTravel')?.addEventListener('click',exportTravel)}
