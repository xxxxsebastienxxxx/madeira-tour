...
async function init(){
  const data=await fetch('data.json?v=8').then(r=>r.json());
...
function fillFilters(){
  const zones=[...new Set(state.places.map(p=>p.zone).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fr'));
  const categories=[...new Set(state.places.map(p=>p.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fr'));
...
function filtered(){
  ...
  return state.places.filter(p=>
    ...
    (!z||p.zone===z)&&(!c||(c==='sunset'?p.sunset===true:p.category===c))&&(!r||String(p.rating)===r)&&
